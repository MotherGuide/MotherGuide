...
<head>
  ...
  <title>Admin — Edit Tip · MotherGuide</title>
...
<main>
  ...
  <div class="page-header">
    ...
    <h1>Edit Tip</h1>
    <p>Update the tip information below.</p>
  </div>

  <div class="glass-card">
    <form id="tip-form">
      <input type="hidden" id="tip_id" name="tip_id" value="<?php echo htmlspecialchars($tip['id']); ?>">
      ...
      <div class="input-group">
        ...
        <div class="input-wrap">
          ...
          <select id="pregnancy_week" name="pregnancy_week" onchange="updateBadge(this.value)">
            <optgroup label="1st Trimester (Weeks 1–12)">
              <?php for ($i=1; $i<=12; $i++): ?>
                <option value="<?php echo $i; ?>" <?php echo $tip['pregnancy_week'] == $i ? 'selected' : ''; ?>>Week <?php echo $i; ?></option>
              <?php endfor; ?>
            </optgroup>
        ...
        <span class="week-preview" id="week-badge">
          <i class="fa-solid fa-baby"></i> Week <?php echo $tip['pregnancy_week']; ?> · <?php echo $tip['pregnancy_week'] <= 12 ? '1st Trimester' : ($tip['pregnancy_week'] <= 26 ? '2nd Trimester' : '3rd Trimester'); ?>
        </span>
      </div>
      ...
      <div class="input-group">
        ...
        <div class="input-wrap">
          ...
          <input type="text" id="title" name="title" value="<?php echo htmlspecialchars($tip['title']); ?>" placeholder="e.g. Eat for Two — But Eat Smart">
        </div>
      </div>
      ...
      <div class="input-group">
        ...
        <div class="input-wrap top-icon">
          ...
          <textarea id="content" name="content" placeholder="Write your tip here. You can use HTML tags like &lt;p&gt;, &lt;ul&gt;, &lt;li&gt;, &lt;strong&gt; etc."><?php echo htmlspecialchars($tip['content']); ?></textarea>
        </div>
        <p class="hint"><i class="fa-solid fa-circle-info"></i> Use &lt;p&gt; for paragraphs and &lt;ul&gt;&lt;li&gt; for bullet lists</p>
      </div>

      <button class="btn-submit" type="submit">
        <i class="fa-solid fa-save"></i> Update Tip
      </button>
...
<script>
  ...
  document.getElementById('tip-form').addEventListener('submit', async function(e) {
    ...
    try {
      const res  = await fetch('api/admin_edit_tip.php', { method:'POST', body: new FormData(e.target) });
      const json = await res.json();
      if (json.status === 'success') {
        msg.className = 'msg success';
        msg.innerHTML = `<i class="fa-solid fa-circle-check"></i> Tip updated successfully`;
      } else {
